...
sed -i 's/Haec/my-project/g' $(grep -rl 'Haec' .)
sed -i 's/HAEC/MY_PROJECT/g' $(grep -rl 'HAEC' .)
sed -i 's/haec/my_project/g' $(grep -rl 'haec' .)
sed -i 's/haec/my-project/g' $(grep -rl 'haec' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `Haec` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `A Turing-complete language whose type discipline is trope-particularity; it lowers programs to the language-neutral Trope IR.` | One-line description | `A tool for X` | flake.nix |
| `HAEC` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `haec` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `haec` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `https://github.com/hyperpolymath/haec` | Project website | `https://example.org` | SECURITY.md |
...
| `A Turing-complete language whose type discipline is trope-particularity; it lowers programs to the language-neutral Trope IR.` | One-line project description | `FFI bridges between languages` | STATE.a2ml |
