...
cookbook:
    ...
    echo "= haec Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/haec.1 << EOF
    .TH haec 1 "$(date +%Y-%m-%d)" "{{version}}" "haec Manual"
    .SH NAME
    haec \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
    ...
    EOF
    echo "Generated: docs/man/haec.1"
...
container-init:
    ...
    read -rp "Service name (e.g. my-api) [haec]: " _SERVICE_NAME
    SERVICE_NAME="${_SERVICE_NAME:-haec}"
...
container-build *args:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman build -t haec:latest -f container/Containerfile .
    elif [ -f "build/Containerfile" ]; then
        podman build -t haec:latest -f build/Containerfile .
    elif [ -f "Containerfile" ]; then
        podman build -t haec:latest -f Containerfile .
    else
...
container-push:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman push haec:latest
    fi

# Run container interactively (for debugging)
container-run *args:
    podman run --rm -it haec:latest {{args}}
