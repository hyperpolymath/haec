...
│       └── include/
│           └── haec.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── haec.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/haec.h
```
...
int main() {
    void* handle = haec_init();
    if (!handle) return 1;

    int result = haec_process(handle, 42);
    if (result != 0) {
        const char* err = haec_last_error();
...
Compile with:
```bash
gcc -o example example.c -lhaec -L./zig-out/lib
```

### From Idris2

```idris
import HAEC.ABI.Foreign
...
#[link(name = "haec")]
extern "C" {
    fn haec_init() -> *mut std::ffi::c_void;
    fn haec_free(handle: *mut std::ffi::c_void);
    fn haec_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = haec_init();
        assert!(!handle.is_null());

        let result = haec_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libhaec = "libhaec"

function init()
    handle = ccall((:haec_init, libhaec), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:haec_process, libhaec), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:haec_free, libhaec), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/haec.h
   ```
