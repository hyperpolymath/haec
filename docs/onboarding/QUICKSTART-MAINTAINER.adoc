...
// Replace rsr-template-repo, haec, {{DEPS}} with actuals
...
Default config location: `$XDG_CONFIG_HOME/haec/config.toml`

Fallback: `$HOME/.config/haec/config.toml`
...
[source,bash]
----
just install --prefix=/opt/haec-instance1 --config=/etc/haec/instance1.toml
just install --prefix=/opt/haec-instance2 --config=/etc/haec/instance2.toml
----
